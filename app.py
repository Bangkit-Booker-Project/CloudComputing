...
def topRatingByGenre(genre):
    ...
    cursor = conn.cursor()
    book = None
    cursor.execute("SELECT * FROM books WHERE bookGenre1=? OR bookGenre2=? OR bookGenre3=? ORDER BY bookRating DESC LIMIT 20", (genre,genre,genre))
    rows = cursor.fetchall()
    listObj = []
    for r in rows:
        book = r
    ...
    if listObj:
        ...
        return "Book with genre {} Not Existing".format(genre), 404
